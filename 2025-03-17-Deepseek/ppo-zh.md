...
注意，对于部分响应的奖励始终为0；只有对于LLM的完整响应，奖励模型才会返回非零标量分数。这一重要事实将在后面变得相关。
...
是不是简单多了？在GRPO中，优势近似为每个响应在其组内响应中的归一化奖励。这消除了critic网络计算每步奖励的需要，更不用说数学上的简单和优雅了。这不禁让人产生疑问——为什么我们没有早点这么做？
